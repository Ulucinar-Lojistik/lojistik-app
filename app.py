...
for col in ["MÜŞTERİ", "DEPO", "ÜRÜNLER", "PLAKA", "DURUM"]:
    if col not in df_aktif.columns: df_aktif[col] = ""
...
if not df_aktif.empty:
    ...
    def renk(row):
        return ['background-color: #d4edda; color: #155724; font-weight: bold;' if 'PLAKA ATANDI' in str(row['DURUM']) else 'background-color: #fff3cd; color: #856404; font-weight: bold;'] * len(row)
...
if yonetici_izni:
    ...
    st.subheader("🧹 Temizlik & Silme İşlemleri")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("🧼 Tamamlananları Temizle"):
            for _, r in df_aktif[df_aktif['DURUM'].str.contains("PLAKA ATANDI")].iterrows():
                veri_gonder("SIL", ["", "", str(r['ÜRÜNLER']), "", ""], search_key=str(r['ÜRÜNLER']))
            st.session_state.sevkiyatlar = veri_cek(); st.rerun()
    with c2:
        sil_sec = st.selectbox("Silinecek Boştaki İş:", [f"{r['MÜŞTERİ']} - {r['ÜRÜNLER']}" for _, r in df_aktif[df_aktif['DURUM'].str.contains("BEKLİYOR")].iterrows()])
        if st.button("🗑️ Seçili Boştaki İşi Sil"):
            if veri_gonder("SIL", ["", "", sil_sec.split(" - ")[-1], "", ""], search_key=sil_sec.split(" - ")[-1]):
                st.session_state.sevkiyatlar = veri_cek(); st.rerun()

    st.subheader("➕ Yeni İş Emri & Atama")
    c1, c2 = st.columns(2)
    with c1:
        mus_sec = st.selectbox("Müşteri:", st.session_state.musteriler)
        urn_sec = st.multiselect("Ürünler:", st.session_state.urunler)
    ...
    with c2:
        ...
        if not bosta.empty:
            is_sec = st.selectbox("Atanacak İş:", [f"{r['MÜŞTERİ']} -> {r['ÜRÜNLER']}" for _, r in bosta.iterrows()])
