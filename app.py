    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if veri and isinstance(veri, list) and len(veri) > 0:
                ...
                # BOŞTAKİLERİ ÜSTE ALMA SIRALAMASI
                df['DURUM_ORDER'] = df['DURUM'].apply(lambda x: 0 if "BEKLİYOR" in str(x).upper() else 1)
                return df.sort_values('DURUM_ORDER').drop(columns=['DURUM_ORDER'])
    except: pass
...
else: st.info("Şu anda aktif iş havuzunda gösterilecek kayıt yok.")
...
if "Yönetici" in rol:
    ...
    if sifre == "1234":
        ...
        # --- SABİT TANIMLAMALAR (TEMİZLEMELİ) ---
        ...
        with col1:
            ...
            if st.button("➕ Şoförü Kaydet"):
                ...
                st.rerun() # Temizlenmiş liste için yeniden çalıştır
        with col2:
        ...
        if st.button("🚀 Veriyi Havuza Gönder"):
            ...
            if veri_gonder("EKLE", [parcalar[0], parcalar[1] if len(parcalar)>1 else parcalar[0], ", ".join(urn_sec), "", "BEKLİYOR (BOŞTA)"]):
                st.success("🎉 İş başarıyla havuza gönderildi!")
                st.rerun()
    else: st.warning("Şifre giriniz (1234).")
else: st.info("Şoför modundasınız. Veri girişi için Yönetici Paneline geçip şifre giriniz.")
